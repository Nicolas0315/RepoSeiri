...
RepoSeiriの現行tool/package versionは`1.0.0`で、machine-readable source contractは`seiri.contract.v5`です。v2 wire内の意味は27個の`contract.semantic_revisions`が所有します。対応するCLI、schema、plugin bundleの変更は[Changelog](../CHANGELOG.md)、[Migration v5](migration-v5.md)、[Release Process](release.md)に記録します。
...
RepoSeiri's current tool/package version is `1.0.0`, while its machine-readable source contract is `seiri.contract.v5`. The 27 `contract.semantic_revisions` own meaning inside the v2 wires. Changes to the corresponding CLI, schemas, and plugin bundle are recorded in the [Changelog](../CHANGELOG.md), [Migration v5](migration-v5.md), and [Release Process](release.md).
